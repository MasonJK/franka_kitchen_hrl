...
if __name__ == '__main__':
    ...
    memory = ReplayBuffer(
        replay_buffer_size, input_size=state_size, n_actions=env.action_space.shape[0])

    try:
        memory.load_from_csv(
            filename=f"checkpoints/human_memory_{task_no_spaces}.npz")
        print(
            f"Successfully loaded human memory file : checkpoints/human_memory_{task_no_spaces}.npz")
    except:
        print(
            f"Could not load human memory file : checkpoints/human_memory_{task_no_spaces}.npz")
    ...
    while True:
        ...
        while not done and episode_steps < max_episode_steps:
            ...
            if (action is not None):
                ...
                print(
                    f"Episode step: {episode_steps}, Reward : {reward}, Successfully added \
