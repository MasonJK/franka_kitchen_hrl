...
class Agent(object):
    def __init__(self, num_inputs, action_space, gamma, tau, alpha, target_update_interval,
        ...
        self.critic = Critic(
            num_inputs, action_space.shape[0], hidden_size, name=f"critic_{goal}").to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=learning_rate)
        self.critic_target = Critic(
            num_inputs, action_space.shape[0], hidden_size, name=f"critic_target_{goal}").to(device=self.device)
        self.hard_update(self.critic_target, self.critic)

        self.policy = Policy(
            num_inputs, action_space.shape[0], hidden_size, action_space, name=f"policy_{goal}").to(device=self.device)
    ...
    def train(self, env, memory: ReplayBuffer, episodes=1000, batch_size=64, update_per_step=1, summary_writer_name="", max_episode_steps=100):
        # Tensorboard
        ...
        for episode in range(episodes):
            ...
            while not done and episode_steps < max_episode_steps:
                ...
                mask = 1 if episode_steps == max_episode_steps else float(
                    not done)
            ...
            print(
                f"Episode: {episode}, Total numsteps: {total_numsteps}, Episode_steps: {episode_steps}, reward: {round(episode_reward, 2)}")
    ...
    def test(self, env: RoboGymObservationWrapper, episodes=1, max_episode_steps=500, prev_action=None):
        for episode in range(episodes):
            ...
            print(
                f"Episode: {episode}, Episode_steps: {episode_steps}, reward: {round(episode_reward, 2)}")
